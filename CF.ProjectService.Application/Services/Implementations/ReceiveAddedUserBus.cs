...
{
   public class ReceiveAddedUserBus
    {
        public static ISubscriptionClient _subscriptionClient;
        private readonly IConfiguration _config;
        private static string topicName = "UserAddedTopic";
        private static string subscriptionName = "completedWithSuccessPrj";
        private static readonly IMediator mediator;

        public ReceiveAddedUserBus(IConfiguration config, ISubscriptionClient subscriptionClient)
        {
            _config = config;
        ...
        public static async Task ProjReceiveMessageAsync()
        {
        ...
        public static async Task ProcessMessagesAsync(Message message, CancellationToken token)
        {
            ...
            var serviceMessageProject = JsonConvert.DeserializeObject<AppUserDto>(messageBody);

            var result = await mediator.Send(new GetUserByEmailQuery { Email = serviceMessageProject.Email });
            if (result == null)
            {
                await mediator.Send(serviceMessageProject);
            }
